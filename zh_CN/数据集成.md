...
来源：[TableModel.h](https://zread.ai/cheng25/KDChart/examples/tools/TableModel.h), [mainwindow.cpp](https://zread.ai/cheng25/KDChart/examples/ModelView/TableView/mainwindow.cpp)
...
来源：[main.cpp#L166-L177](https://zread.ai/cheng25/KDChart/examples/Sql/main.cpp)
...
来源：[TableModel.cpp#L128-L189](https://zread.ai/cheng25/KDChart/examples/tools/TableModel.cpp)
...
db.setDatabaseName(":memory:"); // 或文件路径
db.open();
...
来源:[main.cpp#L152-L178](https://zread.ai/cheng25/KDChart/examples/Sql/main.cpp)
...
dproxy->setSourceModel(sourceModel);
 
// 选择列1和列2作为数据集（跳过列0）
DatasetDescriptionVector columnConfig(2);
columnConfig[0] = 1;  // 使用列1作为第一个数据集
columnConfig[1] = 2;  // 使用列2作为第二个数据集
dproxy->setDatasetColumnDescriptionVector(columnConfig);
 
diagram->setModel(dproxy);
```

来源：[mainwindow.cpp#L117-L126](https://zread.ai/cheng25/KDChart/examples/ModelView/TableView/mainwindow.cpp)
...
来源：[main.cpp#L36-L132](https://zread.ai/cheng25/KDChart/examples/Sql/main.cpp)
...
void MainWindow::itemClicked(const QModelIndex &index)
{
    ...
    m_selectionModel->setCurrentIndex(
        static_cast<const QAbstractProxyModel *>(index.model())->mapToSource(index),
        QItemSelectionModel::Select
    );
}
```

来源：[mainwindow.cpp#L137-L156](https://zread.ai/cheng25/KDChart/examples/ModelView/TableView/mainwindow.cpp)
    ...
    if (file.open(QFile::ReadOnly | QFile::Text)) {
        ...
        while (!stream.atEnd()) {
            ...
            for (int col = 0; col < pieces.size(); ++col) {
                model->setData(model->index(row, col), pieces[col]);
            }
        }
    }
}
...
来源：[mainwindow.cpp#L204-L245](https://zread.ai/cheng25/KDChart/examples/ModelView/TableView/mainwindow.cpp)
...
3. 添加新的数据点:
   ...
   model->setData(model->index(model->rowCount()-1, 0), xValue);
   model->setData(model->index(model->rowCount()-1, 1), yValue);
   ```

当模型发出数据已更改的信号时,图表将自动更新。

来源：[TableModel.cpp#L96-L108](https://zread.ai/cheng25/KDChart/examples/tools/TableModel.cpp)

## 数据集成的最佳实践

1. **选择合适的模型**：根据您的数据源和更新模式匹配模型类型。
2. **使用代理模型**而不是在需要转换视图时复制数据。
...
[<自定义绘制和绘画.md](自定义绘制和绘画.md)							[实时图表.md>](实时图表.md)



[<自定义绘制和绘画](https://zread.ai/cheng25/KDChart/13-custom-drawing-and-painting)							[实时图表>](https://zread.ai/cheng25/KDChart/15-real-time-charting)