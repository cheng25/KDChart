...
// 配置质量控制参数
diagram->setExpectedMeanValue(200);  // 目标值
diagram->setExpectedStandardDeviation(20);  // 允许的变差
...
来源:[main.cpp#L160-L171](https://zread.ai/cheng25/KDChart/examples/LeveyJennings/Simple/main.cpp#L160-L171)
...
```cpp
// 创建一个包含 14 行（数据点）和 6 列的模型
QStandardItemModel model(14, 6);
 
// 设置表头
model.setHeaderData(0, Qt::Horizontal, QObject::tr("批次"));
model.setHeaderData(1, Qt::Horizontal, QObject::tr("值"));
model.setHeaderData(2, Qt::Horizontal, QObject::tr("通过"));
model.setHeaderData(3, Qt::Horizontal, QObject::tr("日期/时间"));
 
// 设置测量点的数据
model.setData(model.index(0, 0), 1);  // 批次 1
model.setData(model.index(0, 1), 210);  // 值
model.setData(model.index(0, 2), true);  // 质控通过
model.setData(model.index(0, 3), 
    QDateTime::fromString("2007-07-06T09:00:00", Qt::ISODate));
```

来源：[main.cpp#L54-L112](https://zread.ai/cheng25/KDChart/examples/LeveyJennings/Simple/main.cpp#L54-L112)
...
// 底轴（显示时间）
auto *axis3 = new KDChart::LeveyJenningsAxis(diagram);
axis3->setPosition(KDChart::CartesianAxis::Bottom);
diagram->addAxis(axis3);
```

来源：[main.cpp#L221-L233](https://zread.ai/cheng25/KDChart/examples/LeveyJennings/Simple/main.cpp#L221-L233)
...
```cpp
// 标记流体包更换
diagram->setFluidicsPackChanges(
    QVector<QDateTime>() << QDateTime::fromString("2007-07-11T15:00:00", Qt::ISODate));
 
// 标记传感器更换
diagram->setSensorChanges(
    QVector<QDateTime>() << QDateTime::fromString("2007-07-10T11:00:00", Qt::ISODate));
```

来源：[main.cpp#L212-L215](https://zread.ai/cheng25/KDChart/examples/LeveyJennings/Simple/main.cpp#L212-L215)
...
// 创建和设置三元坐标系
auto *ternaryPlane = new KDChart::TernaryCoordinatePlane();
chart->replaceCoordinatePlane(ternaryPlane);
 
// 创建三元点图表
auto *diagram = new KDChart::TernaryPointDiagram();
ternaryPlane->replaceDiagram(diagram);
 
// 添加围绕三角形的三个轴
auto *axisA = new KDChart::TernaryAxis(diagram);
axisA->setPosition(KDChartEnums::PositionSouth); // 底部
auto *axisB = new KDChart::TernaryAxis(diagram);
axisB->setPosition(KDChartEnums::PositionWest);  // 左侧
auto *axisC = new KDChart::TernaryAxis(diagram);
axisC->setPosition(KDChartEnums::PositionEast);  // 右侧
 
// 设置数据模型
diagram->setModel(&model);
```

来源：[mainwindow.cpp#L34-L56](https://zread.ai/cheng25/KDChart/examples/TernaryCharts/Advanced/mainwindow.cpp#L34-L56)
...
来源:[mainwindow.cpp#L75-L104](https://zread.ai/cheng25/KDChart/examples/TernaryCharts/Advanced/mainwindow.cpp#L75-L104)
...
对于更复杂的自定义，请参考[样式和外观](https://zread.ai/cheng25/KDChart/10-styling-and-appearance)和[轴和网格](https://zread.ai/cheng25/KDChart/11-axes-and-grids)文档部分，这些部分提供了如何进一步自定义这些特殊图表类型的详细信息。

[<甘特图.md](甘特图.md)						     [样式和外观.md>](样式和外观.md)

[<甘特图](https://zread.ai/cheng25/KDChart/8-gantt-charts)								 [样式和外观>](https://zread.ai/cheng25/KDChart/10-styling-and-appearance)
