...
sequenceDiagram
    ...
    participant Diagram as KDChart::图表
    participant Chart as KDChart::图表
    
    App->>Model: 更新数据值
    Model->>Diagram: dataChanged() 信号
    Diagram->>Chart: 更新视觉表示
    Chart->>App: 显示更新后的图表
```
...
来源：[main.cpp](https://zread.ai/cheng25/KDChart/examples/RealTime/main.cpp)
...
* **批量更新**：如果多个数据点同时变化，考虑使用`beginInsertRows()/endInsertRows()`或类似的模型批量操作。
...
void updateTimeSeriesData()
{
    ...
    m_model.setData(m_model.index(lastRow, 0), QDateTime::currentDateTime());
    m_model.setData(m_model.index(lastRow, 1), getNewDataValue());
    
    // 可选：调整可见范围
    if (m_autoScroll) {
        m_chart.coordinatePlane()->adjustRangesToData();
        // 或者设置固定的可见范围：
        // setVisibleRange(QDateTime::currentDateTime().addSecs(-60), QDateTime::currentDateTime());
    }
}
```
...
```cpp
// 伪代码 距离上次更新的时间 > 最小更新间隔
if (timeSinceLastUpdate > minimumUpdateInterval) {
    m_model.setData(index, newValue);
    lastUpdateTime = currentTime;//上次更新时间 = 当前时间
}
```
...
* 考虑使用KDChart中可用的**数据压缩**技术
* 使用**裁剪**来专注于相关的数据点
* 实施**数据采样**以减少点数，同时保持视觉保真度
...
```cpp
// 连接到您的数据源
connect(dataAcquisitionSystem, &DataAcquisitionSystem::newDataAvailable,
        this, &ChartWidget::onNewDataAvailable);
 
// 处理传入数据
void ChartWidget::onNewDataAvailable(const QVector<double> &newData)
{
    // 更新您的模型
    for (int i = 0; i < newData.size(); ++i) {
        if (i < m_model.columnCount()) {
            m_model.setData(m_model.index(0, i), newData[i]);
        }
    }
}
```
...
```cpp
class CustomDataProvider : public QObject
{
    Q_OBJECT
public:
    CustomDataProvider(QStandardItemModel *targetModel, QObject *parent = nullptr)
        : QObject(parent), m_targetModel(targetModel)
    {
        // 开始数据采集
...
private slots:
    void processNewData(const QByteArray &rawData)
    {
        // 处理原始数据
        double processedValue = extractValue(rawData);
        
        // 更新模型
        QModelIndex index = m_targetModel->index(0, 1);
        m_targetModel->setData(index, processedValue);
...
[<数据集成.md](数据集成.md)					   [Python绑定.md>](Python绑定.md)

[<数据集成](https://zread.ai/cheng25/KDChart/14-data-integration)								[Python 绑定>](https://zread.ai/cheng25/KDChart/16-python-bindings)
